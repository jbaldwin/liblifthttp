...
static auto user_data_on_complete(
    lift::RequestPtr request_ptr, lift::Response response, uint64_t user_data_value1, double user_data_value2) -> void
{
...
TEST_CASE("User data")
{
    ...
    req1->OnCompleteHandler([request_id](lift::RequestPtr request, lift::Response response) {
    ...
    auto req2 = std::make_unique<lift::Request>(
        "http://" + NGINX_HOSTNAME + ":" + NGINX_PORT_STR + "/", std::chrono::seconds{1});
    req2->OnCompleteHandler([request_id](lift::RequestPtr request, lift::Response response) {
