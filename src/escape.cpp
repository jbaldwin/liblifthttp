...
{
    char* escaped_data = curl_escape(data.data(), static_cast<int32_t>(data.length()));
...
{
    char* decoded_data = curl_unescape(escaped_data.data(), static_cast<int32_t>(escaped_data.length()));
